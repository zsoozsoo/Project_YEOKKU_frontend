<template lang="">
	<div>
		...
		<div>
			...
			<input type="radio" id="cycling" value="cycling" v-model="ckMode" />
		...
		<div>
			...
			<input type="radio" id="cycle" value="cycle" v-model="ckWay" />
			<label for="cycle">순환</label>
		</div>
		<!-- 시작점, 도착점 선택 -->
		<div>
			<i v-if="ckWay == 'oneway'">시작점 및 도착점</i>
			<i v-if="ckWay == 'cycle'">시작점</i>
			...
			<i v-if="GET_CURRENT_ROUTE.length > 1 && ckWay == 'cycle'">도착점</i>
		</div>
...
export default {
	...
	computed: {
		...mapGetters('mapStore', ['GET_CURRENT_ROUTE']),
	},
	methods: {
